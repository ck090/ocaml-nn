...
val init_params : int -> int -> int -> ('a t * 'a t * 'a t * 'a t * 'a t * float * 'a t * float)

(* Gradient descent algorithm that takes in various inputs and returns four states *)
val fit : train_x:'a t -> train_y:'a t -> lr:float -> iter:int -> gd_type:Gd.algorithm_type -> optimizer:optim -> activation:Activation.type_ -> beta1:float -> beta2:float -> hidden_units:int -> output_units:int -> epsilon:float -> ('a t * 'a t * 'a t * 'a t) Monad.t
